...
export default function Page() {
  ...
  async function action(prevState, formData) {
    ...
    // Validate form data
    if (!name) {
      newState.nameError = "الاسم مطلوب";
    } else {
    ...
    try {
      ...
      switch (error.code) {
        ...
        case "auth/password-does-not-meet-requirements":
          errorMessage =
            "كلمة المرور لا تلبي المتطلبات, يجب ان تحتوي على 8 أحرف على الأقل و رقم و حرف كبير و حرف صغير";
          break;
        default:
          errorMessage = "خطأ في كلمة المرور" + error;
          break;
      }
  ...
  return (
    <div className="flex flex-col items-center justify-center flex-1 p-4">
      <div className="flex flex-col justify-center items-center max-w-md w-full py-8 px-4 rounded-lg border border-zinc-800">
        <h1 className="text-2xl font-bold mb-4">انشاء حساب</h1>
        ...
        >
          <div className="flex flex-col gap-2">
            <div className="mb-4 flex flex-col gap-1">
              ...
              />
              {formState.nameError && (
                <p className="text-red-500 text-sm">{formState.nameError}</p>
              )}
            </div>
            <div className="mb-4 flex flex-col gap-1">
              <label
          ...
          <div className="flex flex-col gap-2">
            <button
              type="submit"
              disabled={isPending}
              className={`w-full cursor-pointer bg-black text-white border-2 border-white font-semibold py-3 px-4 rounded-lg transition-colors duration-300 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2 ${
                isPending ? "opacity-50 " : "hover:bg-white hover:text-black"
              }`}
            >
              تسجيل{" "}
              {isPending && (
            ...
            <span className="text-center text-gray-400 flex gap-2">
              هل لديك حساب بالفعل؟
              <Link href="/signin" className="hover:underline text-white">
                تسجيل الدخول
              </Link>
            </span>
          </div>
